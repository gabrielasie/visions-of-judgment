...
export default function Nav() {
  ...
  return (
    <>
      <nav className="fixed top-0 left-0 right-0 z-20 h-14 flex items-center justify-between px-4 md:px-8 bg-canvas border-b border-gold/20 overflow-hidden">
        <Link
          href="/"
        ...
        <ul className="flex items-center gap-4 md:gap-7">
          {NAV_LINKS.map(({ href, label }) => (
            <li key={href}>
              <Link
                href={href}
                className={`font-sans text-xs tracking-[0.18em] uppercase transition-colors py-2 px-3 block ${
